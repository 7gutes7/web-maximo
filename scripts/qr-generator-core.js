// Pure JS QR Code Generator based on Kazuhiko Arase's qrcode.js (MIT License)
// Self-contained generator for Valor Máximo QR codes
...
// Export QR creator function
function generateQRSvg(url, options) {
    options = options || {};
    var size = options.size || 512;
    var margin = options.margin !== undefined ? options.margin : 2;
    var darkColor = options.darkColor || "#1b3323"; // Brand dark green / charcoal
    var lightColor = options.lightColor || "#FFFFFF";
    var title = options.title || "";

    var qr = new QRCodeModel(0, 0); // Type auto, M level error correction
    qr.addData(url);
    qr.make();

    var count = qr.getModuleCount();
    var viewSize = count + margin * 2;
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${viewSize} ${viewSize}" width="${size}" height="${size}" shape-rendering="crispEdges">
...
module.exports = { QRCodeModel, generateQRSvg };
